...
const listener = async ({ command, ack, respond }) => {
  ...
  const repo = SERVICES[_product.toLowerCase()].repo || CLIENTS[_product.toLowerCase()].repo
  const product = repo || _product.toLowerCase() || (products.includes(channel) ? channel : 'firstorder')
